...
void SERCOM::initUART(SercomUartMode mode, SercomUartSampleRate sampleRate, uint32_t baudrate)
{
    ...
    sercom->USART.CTRLA.reg = SERCOM_USART_CTRLA_MODE(mode) | SERCOM_USART_CTRLA_SAMPR(sampleRate);
...
{
    //Setting the CTRLA register
    sercom->USART.CTRLA.reg |= SERCOM_USART_CTRLA_TXPO(txPad) | SERCOM_USART_CTRLA_RXPO(rxPad);
...
void SERCOM::initSPI(SercomSpiTXPad mosi, SercomRXPad miso, SercomSpiCharSize charSize, SercomDataOrder dataOrder)
{
    ...
    //Setting the CTRLA register
    //TODO
    sercom->SPI.CTRLA.reg = 3 | //SERCOM_SPI_CTRLA_MODE_SPI_MASTER |
...
void SERCOM::initClockNVIC(void)
{
    ...
    if (sercom == SERCOM0)
    {
        clockId = GCLK_SERCOM0_CORE; // = 18, SERCOM0_GCLK_ID_CORE;
        IdNvic = SERCOM0_IRQn;
    }
    else if (sercom == SERCOM1)
    {
        clockId = GCLK_SERCOM1_CORE;
        IdNvic = SERCOM1_IRQn;
    }
    else if (sercom == SERCOM2)
    {
        clockId = GCLK_SERCOM2_CORE;
        IdNvic = SERCOM2_IRQn;
    }
    else if (sercom == SERCOM3)
    {
        clockId = GCLK_SERCOM3_CORE;
        IdNvic = SERCOM3_IRQn;
    }
    else if (sercom == SERCOM4)
    {
        clockId = GCLK_SERCOM4_CORE;
        IdNvic = SERCOM4_IRQn;
    }
    else if (sercom == SERCOM5)
    {
        clockId = GCLK_SERCOM5_CORE;
        IdNvic = SERCOM5_IRQn;
    }
    ...
    // Setting clock
    GCLK->PCHCTRL[clockId].bit.CHEN = 0;
    GCLK->PCHCTRL[clockId].reg = (GCLK_PCHCTRL_CHEN | GCLK_PCHCTRL_GEN_GCLK0);
    while (!(GCLK->PCHCTRL[clockId].reg & GCLK_PCHCTRL_CHEN))
    {
        /* Wait for clock synchronization */
    }

    //while ( GCLK->SYNCBUSY.reg & GCLK_SYNCBUSY_GENCTRL( GCLK_PCHCTRL_GEN_GCLK0 ) );
