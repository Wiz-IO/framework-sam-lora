...
#if 1
  umm_heap = (umm_block *)&_heap_begin;
  umm_numblocks = ((uint32_t)&_heap_size / sizeof(umm_block));
  memset(umm_heap, 0x00, (uint32_t)&_heap_size);
#else
  /* init heap pointer and size, and memset it to 0 */
...
void *malloc(size_t size)               __attribute__((alias("umm_malloc")));
void *calloc(size_t num, size_t size)   __attribute__((alias("umm_calloc")));
void *realloc(void *ptr, size_t size)   __attribute__((alias("umm_realloc")));
void free(void *ptr)                    __attribute__((alias("umm_free")));
